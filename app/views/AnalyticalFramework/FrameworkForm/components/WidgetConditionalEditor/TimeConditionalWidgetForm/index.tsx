...
import SimpleTimeConditionInput from './SimpleTimeConditionInput';

import styles from './styles.css';

interface Option {
    key: TimeCondition['operator'],
    label: string,
...
type PartialConditionTypeNew = PartialForm<
    TimeAfterCondition | TimeBeforeCondition | TimeEqualToCondition,
    'operator' | 'conjunctionOperator' | 'key' | 'order'
>;
...
const conditionSchema: ConditionSchema = {
    fields: (val): ConditionSchemaFields => {
        ...
        if (
            val.operator === 'time-after'
            || val.operator === 'time-before'
            || val.operator === 'time-equal-to'
        ) {
            return {
                ...basicValidation,
...
function ConditionInput(props: ConditionInputProps) {
    ...
    return (
        ...
        >
            ...
            />
            {(
                value.operator === 'time-after'
                || value.operator === 'time-before'
                || value.operator === 'time-equal-to'
            ) && (
                <SimpleTimeConditionInput
                    index={index}
                    value={value}
...
interface TimeConditionalWidgetFormProps {
    onCancel: () => void;
...
function TimeConditionalWidgetForm(props: TimeConditionalWidgetFormProps) {
    const {
        onSave,
        onCancel,
        initialValue,
...
export default TimeConditionalWidgetForm;
