...
import SimpleTextConditionInput from './SimpleTextConditionInput';

import styles from './styles.css';

interface Option {
    key: TextCondition['operator'],
    label: string,
    invertedLabel: string,
}

const options: Option[] = [
    { key: 'text-starts-with', label: 'Starts with', invertedLabel: 'Does not start with' },
    { key: 'text-ends-with', label: 'Ends with', invertedLabel: 'Does not end with' },
    { key: 'text-contains', label: 'Contains', invertedLabel: 'Does not contain' },
...
const conditionSchema: ConditionSchema = {
    fields: (val): ConditionSchemaFields => {
        ...
        if (
            val.operator === 'text-starts-with'
            || val.operator === 'text-ends-with'
            || val.operator === 'text-contains'
        ) {
            return {
                ...basicValidation,
...
function ConditionInput(props: ConditionInputProps) {
    ...
    return (
        ...
        >
            ...
            />
            {(
                value.operator === 'text-contains'
                || value.operator === 'text-ends-with'
                || value.operator === 'text-starts-with'
            ) && (
                <SimpleTextConditionInput
                    index={index}
                    value={value}
...
interface TextConditionalWidgetFormProps {
    onCancel: () => void;
...
function TextConditionalWidgetForm(props: TextConditionalWidgetFormProps) {
    const {
        onSave,
        onCancel,
        initialValue,
...
export default TextConditionalWidgetForm;
