...
const LINE_CHART_ENUMS = gql`
    query ReportLineChartEnums {
        enums {
...
export const defaultLineChartValue: LineChartConfigType = {
...
interface Props<NAME extends string> {
    name: NAME;
    className?: string;
    value: LineChartConfigType | undefined;
    onChange: (value: SetValueArg<LineChartConfigType | undefined>, name: NAME) => void;
    error?: Error<LineChartConfigType>;
    disabled?: boolean;
...
function LineChartChartEdit<NAME extends string>(props: Props<NAME>) {
    const {
        className,
        value,
        onChange,
    ...
    const {
        data: enumsData,
    } = useQuery<ReportLineChartEnumsQuery>(
        LINE_CHART_ENUMS,
    );
    ...
    const handleAddVerticalAxis = useCallback(() => {
        onFieldChange(
            (oldValue: LineChartConfigType['verticalAxis']) => {
...
export default LineChartChartEdit;
